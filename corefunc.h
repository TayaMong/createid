	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("add_label_with_icon|big|`wGet a GrowID``|left|206|") != std::string::npos) {
	    				SendPacket(2, "action|dialog_return\ndialog_name|growid_apply\nlogon|"+ prozvishche + to_string(Apee_Luu) + "\npassword|" + gaghtnabarry + "\npassword_verify|" + gaghtnabarry + "\nemail|"+ jadinya + "@gmail.com", peer);
	                Tau_Ah = Tau_Ah + 1;
	    			}
	    				...
	    				string passwr = ""+ gaghtnabarry;
	    				cout << "========================================\n©TayaGabutz: Success Make Account!\n©TayaGabutz: GrowID : " << growid << "\n©TayaGabutz: Password : " << gaghtnabarry << "\n========================================" << endl;
	    				appendAcc("User: " + growid + ",Password: " + gaghtnabarry + ",Email: " + jadinya + "@gmail.com");
	                    Apee_Luu = Apee_Luu + 1;
	                    if (Tau_Ah >= 1)
	                    {
		                jadinya = Scrisoare_moderna + to_string(Apee_Luuu);
	                    Apee_Luuu = Apee_Luuu + 1;
	                    }
	                    if (Jumlah_Growid >= Limit_Growid) {
                        exit(3);
                        }
                        Jumlah_Growid = Jumlah_Growid +1;
